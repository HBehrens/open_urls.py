...
class Tests(unittest.TestCase):
    def test_line_is_url(self):
        self.assertEqual(find_urls('http://just.one.url.in.line.com'), {
            'http://just.one.url.in.line.com',
        })

    def test_url_surrounded_by_text(self):
        self.assertEqual(find_urls('before http://and.net after'), {
            'http://and.net',
        })

    def test_two_urls_in_line(self):
        self.assertEqual(find_urls('a https://two.com b http://urls.in.line c'), {
            'https://two.com', 'http://urls.in.line',
        })

    def test_http_augmented(self):
        self.assertEqual(find_urls('yes heikobehrens.net does work'), {
            'http://heikobehrens.net',
        })

    def test_tasks(self):
        self.assertEqual(find_tasks('before T1234 after'), {
            'https://tasks/1234',
        })
    ...
    def test_process_file(self, subp_call):
        ...
        self.assertEqual(subp_call.call_count, 4)
        subp_call.assert_has_calls([
    ...
    def test_process_file_exits(self, subp_call):
        ...
        self.assertEqual(subp_call.call_count, 1)
        subp_call.assert_has_calls([
            call('some command f1:line1', shell=True),
        ])

    @patch('subprocess.call')
    @patch('open_urls.print')
    def test_process_file_dry_run(self, sys_print, subp_call):
        ...
        self.assertEqual(sys_print.call_count, 1)
        sys_print.assert_has_calls([
